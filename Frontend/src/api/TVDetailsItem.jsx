...
const TVDetailsItem = (props) => {
  ...
  const checkFavorite = () => {
    const url = `https://api.themoviedb.org/3/account/20465724/favorite/movies?language=en-US&page=1&session_id=${localStorage.getItem(
      "SessionID"
    ...
    fetch(url, options)
      ...
      .then((json) => {
        ...
        const isMovieLiked = json.results.some(
  ...
  const Favorites = () => {
    const url = `https://api.themoviedb.org/3/account/20465724/favorite?session_id=${localStorage.getItem(
      "SessionID"
    )}`;
    const options = {
      method: "POST",
  ...
  const playTrailers = () => {
    const url = `https://api.themoviedb.org/3/tv/${props.id}/videos?language=en-US`;
    const options = {
      method: "GET",
      headers: {
        accept: "application/json",
  ...
  const theCastAPI = () => {
    const url = `https://api.themoviedb.org/3/tv/${props.id}/credits?language=en-US`;
    const options = {
      method: "GET",
      headers: {
        accept: "application/json",
  ...
  return (
    <div className="max-w-[1640px] mx-auto sm:max-w-screen-xl mt-10 md:mt-20 border-b border-gray-500 pb-4 flex flex-col md:flex-row items-center p-3">
      ...
      <div className="flex flex-col md:ml-10 md:lg:ml-20 md:mt-0 md:w-2/3 text-center md:text-left">
        {/* Movie Title */}
        <h1 className="text-3xl md:text-4xl lg:text-5xl font-semibold mt-3 md:mt-0">
          {props.title}
        </h1>
        ...
        <div className="flex justify-center md:justify-start items-center text-gray-400 text-sm mt-2 md:mt-5">
          ...
          <span className="ml-4">Genre: {props.genre}</span>
        </div>

        {/* Movie Overview */}
        <p className="mt-5 text-gray-300 text-md md:text-lg">
          {props.overview}
        </p>
        ...
        <div className="mt-5 flex flex-col md:flex-row justify-center md:justify-start">
          ...
          <button
            onClick={handleLike}
            className="ml-3 mt-5 sm:mt-[-10px] rounded-md bg-gradient-to-r from-gray-500 to-gray-700 hover:from-gray-700 hover:to-gray-900 text-white px-5 py-2 font-semibold transition duration-300 ease-in-out"
          >
            {getButtonText()}
          </button>
        </div>
